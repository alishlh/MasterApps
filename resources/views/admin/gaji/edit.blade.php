    ...
    <div class="card">
      <div class="card-body">
        <h5 class="card-title fw-semibold mb-4">Form Gaji</h5>
          ...
          <div class="card-body">
            <form action="{{url('admin/gaji/update/'. $item->id)}}" method="POST">
                @csrf
                <div class="mb-3">
                  <label for="name" class="form-label">Gaji</label>
                  <input type="text" class="form-control" id="name" name="name" value="{{$item->user->name}}" required>
              </div>

              <div class="mb-3">
                <label for="jabatan" class="form-label">Gaji</label>
                <input type="text" class="form-control" id="jabatan" name="jabatan" value="{{$item->jabatan->jabatan}}" required>
            </div>
                  <div class="mb-3">
                    <label for="gaji" class="form-label">Gaji</label>
                    <input type="number" class="form-control" id="gaji" name="gaji" value="{{$item->gaji}}" required>
                </div>

                <div class="mb-3">
                    <label for="noRek" class="form-label">Nomor Rekening</label>
                    <input type="text" class="form-control" id="noRek" name="noRek" value="{{$item->noRek}}" required>
                </div>
